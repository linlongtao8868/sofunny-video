...
| `--video-input` | 否 | 可重复，参考视频本地路径；编码为 `data:video/...;base64,...` 放入 `metadata.content[].video_url` |
| `--audio-input` | 否 | 可重复，参考音频本地路径；编码为 `data:audio/...;base64,...` 放入 `metadata.content[].audio_url`，带 `role:"reference_audio"` |
