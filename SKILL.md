...
- 推荐安装：`npx skills add https://github.com/linlongtao8868/sofunny-video.git`，安装时可自主选择目标 Agent（Claude Code / Codex 等）的 skills 路径。
- 可执行脚本入口：`<安装目录>/scripts/sofunny-video.js`（下文示例以 `${SKILL_DIR}` 代指该安装目录）。
...
更多示例（多参考图、公网 URL 透传、指定时长/画幅/分辨率、HappyHorse 模型）见 [references/api.md](references/api.md#更多示例)。
